...
L'intégration continue… Vaste chantier en perspective.
...
On commence par fortement **diminuer les erreurs** possibles via l'automatisation.
...
Les erreurs et régressions sont aussi **plus vite détectées**.
...
**On vérifie aussi beaucoup plus de choses.**
...
Le principe est extrèmement simple : **l'intégration continue casse, on arrête tout et on corrige !**.
...
Ce n'est pas parce que la machine va s'occuper de toutes les taches ingrates que l'homme n'a plus rien à faire !
...
Au passage, on peut en profiter pour détecter les faiblesses de chaque développeur et homogénéiser l'équipe.
...
OK, tout le monde n'a pas le niveau des speakers de la Devoxx ou ne travaille pas tous les matins sur un projet opensource techniquement encadré par des pointures du développement. Mais quand même, faut pas pousser…
...
Au début du monde, certes, le projet semble avancer, les fonctionnalités arrivent, tout le monde est content.
...
À ce moment, on passe généralement en mode panique, et on fait arriver tous les seniors et architectes, qui se rendent compte de la catastrophe technique qui s'est jouée sournoisement dans leurs dos.
...
Tout ça pour en arriver à ma conclusion.
...
Mettre en place de la CI implique donc aussi de **former les équipes à tous les outils nécessaires.**
